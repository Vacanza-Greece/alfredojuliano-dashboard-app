...
> = {
  ...
  FAILED: { text: "Failed", bg: "bg-red-50", textColor: "text-red-700" },
  PENDING: { text: "Pending", bg: "bg-amber-50", textColor: "text-amber-700" },
  REFUNDED: {
    text: "Refunded",
    bg: "bg-purple-50",
  ...
  <div className="flex flex-col md:flex-row md:items-center md:justify-between gap-4">
    <div>
      ...
      <p className="text-black font-semibold mt-2">
        Payment ID: <span className="text-gray-600">{id}</span>
      </p>
    </div>
    <Badge
...
const PaymentInfoCard = ({ payment }: { payment: Payment }) => (
  <Card>
    ...
    <CardContent className="space-y-3">
      ...
      <InfoRow
        label="Invoice"
        value={`INV-${payment.id.slice(0, 8).toUpperCase()}`}
      />
    </CardContent>
  </Card>
);
...
const SubscriptionInfoCard = ({
  ...
  <Card>
    ...
    <CardContent className="space-y-3">
      ...
      <InfoRow
        label="Price"
        value={`${subscription.plan.price} ${currency.toUpperCase()}`}
      />
      <InfoRow
        label="Period"
        value={`${format(
...
const FeatureItem = ({
  ...
  <div className="flex items-start gap-3">
    <span className="h-5 w-5 text-emerald-500 mt-0.5">✓</span>
    <div>
      <p className="font-medium">{feature}</p>
      <p className="text-sm text-muted-foreground">Included in {planName}</p>
    </div>
  </div>
);
...
export function PaymentDetails() {
  ...
  if (isError)
    return (
      <ErrorState message="Failed to load payment details. Try again later." />
    );
  ...
  return (
    <div className="mx-auto space-y-6">
      <Button
        variant="outline"
        size="sm"
        className="bg-[var(--color-blueOne)] hover:bg-[var(--color-accent)] hover:text-white text-white mb-6 cursor-pointer"
      ...
      <PaymentHeader id={payment.id} status={payment.status} />
