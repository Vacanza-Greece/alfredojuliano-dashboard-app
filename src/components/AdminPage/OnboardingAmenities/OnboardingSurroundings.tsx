...
const OnboardingSurroundings = () => {
  ...
  const handleOpenModal = (surrounding?: Surrounding) => {
    if (surrounding) {
      ...
      setFormData({
        ...
        greek_name: surrounding.greek_name,
        icon: null,
      });
    } else {
      setIsEditMode(false);
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    if (!formData.greek_name.trim()) return setError("Greek name is required");
  ...
  const handleConfirmDelete = async () => {
    ...
    try {
      ...
      toast.success("Surrounding deleted successfully 🗑️");
      setDeleteModalOpen(false);
      setSurroundingToDelete(null);
    } catch (err: any) {
      const message = err?.data?.message || "Failed to delete surrounding";
      setError(message);
      toast.error(message);
    }
  };
