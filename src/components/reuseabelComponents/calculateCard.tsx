...
const CalculateCard = () => {
  ...
  const { data: propertiesData } = useGetPropertiesQuery({ limit: 1000 } as any);
  ...
  const planCount = (plans as any)?.meta?.total ?? plans?.length ?? 0;
  const paymentCount = payments?.length ?? 0;
  const propertyCount = Math.max(
    properties.length,
    (propertiesData as any)?.meta?.total ?? 0
  );
  ...
  const propertiesLast30 = getLast30DaysData(properties);
  ...
  const statusData = [
    ...
    },
    {
      ...
      amount: `${propertyCount} Property`,
      change: getChange(propertiesLast30.length, propertyCount),
      unit: `${propertiesLast30.length} added`,
